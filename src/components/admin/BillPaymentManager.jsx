...
export default function BillPaymentManager() {
  const [data, setData] = useState({ columns: 5, items: [] });
    ...
    <>
      <div className="admin-section">
        <div className="admin-section__header">
          <h2 className="admin-section__title">Quản lý Thanh toán hóa đơn</h2>
        ...
        {data.items.length === 0 ? (
          <div className="empty-state">
            ...
            <div className="empty-state__text">Chưa có mục thanh toán nào</div>
          </div>
        ) : (
              ...
              {data.items
                ...
                .map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>{item.name}</td>
                    <td><span className="admin-table__color" style={{ backgroundColor: item.color }} /></td>
                    <td>{item.order}</td>
                    <td>
        ...
        <div className="modal-overlay" onClick={resetForm}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal__header">
              <h3 className="modal__title">{editItem ? '✏️ Sửa' : '+ Thêm mục thanh toán'}</h3>
            ...
            <form onSubmit={handleSubmit} className="admin-form">
              <div className="form-group">
                <label>Tên *</label>
                <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="VD: Điện" required />
              </div>
